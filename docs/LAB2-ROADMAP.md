...
Favicon Lab2: dos forks activos (`@itsliaaa/baileys` y `@itsmelody/Baileys`) documentan y codifican una ruta distinta al `linkPreview` simple: además de `highQualityThumbnail`, aceptan un campo superior `favicon: { url }` y lo convierten internamente a `extendedTextMessage.faviconMMSMetadata`. WaSocket todavía no mapea `favicon`, así que Lab2 replica esa salida de forma local solo para esta prueba de Instagram: prepara el favicon con `prepareWAMessageMedia`, arma `faviconMMSMetadata` y manda un `ExtendedTextMessage` estándar por `relayMessage`. Por pedido de prueba, `.richdemo ig` usa el favicon de OnlyFans como fuente del ícono; si no puede prepararlo, cae al preview grande ya confirmado.
