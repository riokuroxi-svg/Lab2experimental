...
Conclusión favicon Lab2: `link-preview-js` expone `favicons`, pero WaSocket/Baileys `getUrlInfo()` no los reexpone y el generador normal de `sendMessage` no mapea ningún campo de favicon al `ExtendedTextMessage`. Sin evidencia sólida de que `faviconMMSMetadata` sea respetado de forma confiable por clientes reales, no se deja código especulativo para forzar el ícono. Nos quedamos con el preview grande con imagen, que ya fue confirmado en pruebas reales.
